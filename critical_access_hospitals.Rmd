...
cahs_all %>%
  ...
  geom_bar() +
  labs(title = "Critical Access Hospitals by Number of Beds",
       subtitle = "Data from Flex Monitoring Team, 2007-20018", 
       x = "Number of Beds",
       y = "Number of CAHs") +
  facet_wrap(~year) +  
  theme_calc()
...
tx_match_2019 %>%
  filter(total_beds < 51) %>%
  ggplot(aes(x = total_beds, fill = critical_access)) +
  geom_bar() +
  labs(title = "Hospital Beds in Texas Hospitals",
  ...
  scale_fill_calc(labels = c("Not CAH", "Critical Access Hospital")) +
  theme_calc()
